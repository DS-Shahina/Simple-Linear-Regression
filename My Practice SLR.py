...
wcat = pd.read_csv("D:/C DRIVE-SSD DATA backup 15-12-2020/Desktop/360digitmg material/Simple Linear Regression/wc-at.csv")
...
plt.bar(height = wcat.AT, x = np.arange(1, 110, 1)) #numpy.arange([start, ]stop, [step, ], dtype=None) -> numpy.ndarray
...
# correlation
np.corrcoef(wcat.Waist, wcat.AT) #0.81855781 - Moderate Relation
...
# Equation is, y = -215.9815+3.4589(Waist) -> B0+B1x
...
# Error calculation
res1 = wcat.AT - pred1 # Actual - Predicted
res_sqr1 = res1 * res1 # r square
mse1 = np.mean(res_sqr1)
rmse1 = np.sqrt(mse1)
rmse1 #32.76017749575515
...
np.corrcoef(np.log(wcat.Waist), wcat.AT) #correlation = 0.82177819

model2 = smf.ols('AT ~ np.log(Waist)', data = wcat).fit()
model2.summary() #R-squared:0.675
...
np.corrcoef(wcat.Waist, np.log(wcat.AT)) #correlation = 0.84090069

model3 = smf.ols('np.log(AT) ~ Waist', data = wcat).fit()
model3.summary() #R-squared: 0.707
...
rmse3 = np.sqrt(mse3)
rmse3 #38.52900175807141


#### Polynomial transformation
# x = waist; x^2 = waist*waist; y = log(at)

model4 = smf.ols('np.log(AT) ~ Waist + I(Waist*Waist)', data = wcat).fit() # y ~ X + I X^2 + I X^3
model4.summary() #R-squared: 0.779
...
#Polynomial model  32.244448 gives least error
...
finalmodel.summary() #R-squared:0.771
